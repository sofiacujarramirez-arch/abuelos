"use client";

import { useRef, useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { ImagePlus, X, Loader2 } from "lucide-react";
import { createClient } from "@/lib/supabase/client";

const MIN_PHOTOS = 3;
const MAX_PHOTOS = 5;
const MAX_MB = 15;

export type ExistingPhoto = {
  path: string;
  url: string;
  width?: number | null;
  height?: number | null;
};

type UnifiedPhoto =
  | { kind: "existing"; path: string; url: string; width?: number | null; height?: number | null }
  | { kind: "new"; file: File; url: string };

export type PostEditorSubmitArgs = {
  message: string;
  eventDate: string | null;
  status: "draft" | "published";
  photos: { path: string; width?: number | null; height?: number | null }[];
};

export type PostEditorResult = { error: string } | { ok: true; redirectTo?: string };

export function PostEditor({
  familyId,
  initialMessage = "",
  initialEventDate = null,
  initialStatus = "draft",
  initialPhotos = [],
  submitLabel = "Publicar en la gaceta",
  onSubmit,
  allowDraft = true,
}: {
  familyId: string;
  initialMessage?: string;
  initialEventDate?: string | null;
  initialStatus?: "draft" | "published";
  initialPhotos?: ExistingPhoto[];
  submitLabel?: string;
  onSubmit: (args: PostEditorSubmitArgs) => Promise<PostEditorResult>;
  allowDraft?: boolean;
}) {
  const router = useRouter();
  const fileInput = useRef<HTMLInputElement>(null);
  const [photos, setPhotos] = useState<UnifiedPhoto[]>(
    initialPhotos.map((p) => ({ kind: "existing", ...p })),
  );
  const [message, setMessage] = useState(initialMessage);
  const [eventDate, setEventDate] = useState(initialEventDate ?? "");
  const [error, setError] = useState<string>();
  const [uploading, setUploading] = useState(false);
  const [pendingStatus, setPendingStatus] = useState<"draft" | "published" | null>(null);
  const [pending, startTransition] = useTransition();

  function handleFiles(files: FileList | null) {
    if (!files) return;
    setError(undefined);
    const remaining = MAX_PHOTOS - photos.length;
    const toAdd = Array.from(files).slice(0, remaining);
    const next: UnifiedPhoto[] = [];
    for (const file of toAdd) {
      if (!file.type.startsWith("image/")) {
        setError("Solo imágenes, por favor.");
        continue;
      }
      if (file.size > MAX_MB * 1024 * 1024) {
        setError(`Cada foto máximo ${MAX_MB}MB.`);
        continue;
      }
      next.push({ kind: "new", file, url: URL.createObjectURL(file) });
    }
    setPhotos((p) => [...p, ...next]);
    if (fileInput.current) fileInput.current.value = "";
  }

  function removePhoto(idx: number) {
    setPhotos((p) => {
      const ph = p[idx];
      if (ph.kind === "new") URL.revokeObjectURL(ph.url);
      return p.filter((_, i) => i !== idx);
    });
  }

  async function submit(targetStatus: "draft" | "published") {
    setError(undefined);

    if (targetStatus === "published" && photos.length < MIN_PHOTOS) {
      setError(`Para publicar sube al menos ${MIN_PHOTOS} fotos.`);
      return;
    }
    if (photos.length > MAX_PHOTOS) {
      setError(`Máximo ${MAX_PHOTOS} fotos.`);
      return;
    }
    if (targetStatus === "published" && !message.trim()) {
      setError("Escribe un mensaje para la abuela.");
      return;
    }

    setUploading(true);
    setPendingStatus(targetStatus);
    const supabase = createClient();
    const { data: userRes } = await supabase.auth.getUser();
    const userId = userRes.user?.id;
    if (!userId) {
      setError("Tu sesión expiró.");
      setUploading(false);
      setPendingStatus(null);
      return;
    }

    const finalPhotos: PostEditorSubmitArgs["photos"] = [];
    try {
      const timestamp = Date.now();
      let newIndex = 0;
      for (const ph of photos) {
        if (ph.kind === "existing") {
          finalPhotos.push({ path: ph.path, width: ph.width ?? null, height: ph.height ?? null });
        } else {
          const ext = ph.file.name.split(".").pop()?.toLowerCase() || "jpg";
          const path = `${familyId}/${userId}/${timestamp}-${newIndex}.${ext}`;
          const { error: upErr } = await supabase.storage
            .from("family-photos")
            .upload(path, ph.file, { contentType: ph.file.type, upsert: false });
          if (upErr) throw new Error(upErr.message);
          finalPhotos.push({ path, width: null, height: null });
          newIndex++;
        }
      }
    } catch (err) {
      setError(`Falló la subida: ${err instanceof Error ? err.message : "error"}`);
      setUploading(false);
      setPendingStatus(null);
      return;
    }
    setUploading(false);

    startTransition(async () => {
      const result = await onSubmit({
        message: message.trim(),
        eventDate: eventDate || null,
        status: targetStatus,
        photos: finalPhotos,
      });
      if ("error" in result) {
        setError(result.error);
        setPendingStatus(null);
      } else if (result.redirectTo) {
        router.push(result.redirectTo);
      } else {
        router.refresh();
      }
    });
  }

  const busy = uploading || pending;
  const countOk = photos.length >= MIN_PHOTOS && photos.length <= MAX_PHOTOS;
  const canPublish = countOk && message.trim().length > 0;
  const canDraft = photos.length <= MAX_PHOTOS && (photos.length > 0 || message.trim().length > 0);

  return (
    <form onSubmit={(e) => { e.preventDefault(); submit("published"); }} className="space-y-8">
      <div>
        <div className="flex items-baseline justify-between mb-3">
          <label className="font-body text-base font-semibold text-inkwell">
            Fotos <span className="font-normal italic text-tobacco">(entre {MIN_PHOTOS} y {MAX_PHOTOS})</span>
          </label>
          <span className={`font-body text-sm italic ${countOk ? "text-garden" : "text-tobacco"}`}>
            {photos.length} / {MAX_PHOTOS}
          </span>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {photos.map((p, i) => (
            <div key={p.kind === "existing" ? p.path : p.url} className="relative aspect-square bg-cream border-2 border-inkwell/10 overflow-hidden group">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={p.url} alt="" className="w-full h-full object-cover" />
              <button
                type="button"
                onClick={() => removePhoto(i)}
                disabled={busy}
                className="absolute top-2 right-2 bg-inkwell/80 text-parchment rounded-full p-1.5 opacity-0 group-hover:opacity-100 focus:opacity-100 transition disabled:opacity-30"
                aria-label="Quitar foto"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
          ))}

          {photos.length < MAX_PHOTOS && (
            <button
              type="button"
              onClick={() => fileInput.current?.click()}
              disabled={busy}
              className="aspect-square border-2 border-dashed border-inkwell/30 hover:border-inkwell bg-white flex flex-col items-center justify-center gap-2 font-body text-tobacco hover:text-inkwell transition disabled:opacity-50"
            >
              <ImagePlus className="w-8 h-8" />
              <span className="italic text-sm">Agregar foto</span>
            </button>
          )}
        </div>

        <input
          ref={fileInput}
          type="file"
          accept="image/*"
          multiple
          onChange={(e) => handleFiles(e.target.files)}
          className="hidden"
        />
      </div>

      <label className="block">
        <span className="font-body text-base font-semibold text-inkwell mb-2 block">
          Mensaje para la abuela
        </span>
        <textarea
          value={message}
          onChange={(e) => setMessage(e.target.value)}
          maxLength={500}
          rows={5}
          className="w-full bg-white border-2 border-inkwell/15 focus:border-inkwell px-4 py-3 font-body text-body focus:outline-none transition resize-none"
          placeholder="Abuelita, estas fotos son del cumpleaños de Mateo. Te extrañamos mucho y..."
        />
        <span className="font-body italic text-sm text-tobacco mt-1 block text-right">
          {message.length} / 500
        </span>
      </label>

      <label className="block max-w-xs">
        <span className="font-body text-base font-semibold text-inkwell mb-2 block">
          Fecha del evento <span className="font-normal italic text-tobacco">(opcional)</span>
        </span>
        <input
          type="date"
          value={eventDate}
          onChange={(e) => setEventDate(e.target.value)}
          className="w-full bg-white border-2 border-inkwell/15 focus:border-inkwell px-4 py-3 font-body text-body focus:outline-none transition"
        />
      </label>

      {error && (
        <div className="bg-envelope-tint border-l-4 border-envelope p-4 font-body text-body text-envelope">
          {error}
        </div>
      )}

      <div className="flex flex-wrap gap-4 pt-4 border-t-2 border-inkwell/10">
        <button
          type="button"
          onClick={() => router.back()}
          disabled={busy}
          className="btn-ghost"
        >
          Cancelar
        </button>
        {allowDraft && initialStatus !== "published" && (
          <button
            type="button"
            onClick={() => submit("draft")}
            disabled={busy || !canDraft}
            className="btn-secondary disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {busy && pendingStatus === "draft" ? <><Loader2 className="w-4 h-4 animate-spin" /> Guardando...</> : "Guardar borrador"}
          </button>
        )}
        <button
          type="submit"
          disabled={busy || !canPublish}
          className="btn-primary flex-1 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {uploading && pendingStatus === "published" && <><Loader2 className="w-4 h-4 animate-spin" /> Subiendo fotos...</>}
          {pending && pendingStatus === "published" && <><Loader2 className="w-4 h-4 animate-spin" /> {initialStatus === "published" ? "Guardando..." : "Publicando..."}</>}
          {!busy && (initialStatus === "published" ? "Guardar cambios" : submitLabel)}
        </button>
      </div>
    </form>
  );
}
